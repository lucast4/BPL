...
%
function [bestM_pass2,score_sorted] = parses_to_MPs_LT(I,PP,ninit,lib,verbose,pick_random,do_optimize)
    ...
    if pick_random
        [bestM_pass1,score_sorted,score_indx] = pick_K_random(MP,lib,K_order);
    else
...
%     end
    ...
    if pick_random
        [bestM_pass2,score_sorted,score_indx] = pick_K_random(bestM_pass1,lib,ninit);
    else    
...
% Pick K random, equal prob
function [bestM,score_sorted,score_indx] = pick_K_random(list_M,lib,K)
    n = numel(list_M);
    scores = zeros(n, 1)
    npick = min(n,K);
    [score_sorted,score_indx] = randsort(scores,'descend');
...
% Pick K best, based only on image score
function [bestM,score_sorted,score_indx] = pick_K_imageonly(list_M,lib,K)
    n = numel(list_M);
    scores = score_set_imageonly(list_M,lib);
    npick = min(n,K);
    [score_sorted,score_indx] = randsort(scores,'descend');
    pick = score_indx(1:npick);
    bestM = list_M(pick);
...
function scores = score_set_imageonly(list_M,lib)
    n = numel(list_M);
    scores = -inf(n,1);
    for i=1:n
        scores(i) = scoreMP_NoRel(list_M{i},lib,...
                    'token',false,'type',false,'image',false);
    end
end